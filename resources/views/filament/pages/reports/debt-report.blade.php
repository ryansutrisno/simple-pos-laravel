<x-filament-panels::page>
    <div class="space-y-6">
        ...
        @if(!empty($reportData))
                    ...
                    <x-filament::card>
                        <div class="text-center">
                            ...
                            <div class="text-2xl font-bold text-red-600">
                                Rp {{ number_format($reportData['total_debt'], 0, ',', '.') }}
                            </div>
                        </div>
                    </x-filament::card>
                </div>
                    ...
                    <x-filament::card>
                        <div class="text-center">
                            ...
                            <div class="text-2xl font-bold text-green-600">
                                Rp {{ number_format($reportData['total_paid'], 0, ',', '.') }}
                            </div>
                        </div>
                    </x-filament::card>
                </div>
                    ...
                    <x-filament::card>
                        <div class="text-center">
                            ...
                            <div class="text-2xl font-bold text-orange-600">
                                Rp {{ number_format($reportData['total_overdue'], 0, ',', '.') }}
                            </div>
            ...
            @if(!empty($reportData['aging_report']))
                <x-filament::card>
                    <h3 class="text-lg font-semibold mb-4">Aging Report</h3>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        @foreach($reportData['aging_report'] as $category => $data)
                            <div class="text-center p-4 {{ $data['category'] === 'Overdue' ? 'bg-red-50' : 'bg-green-50' }} rounded-lg">
                                <div class="font-semibold {{ $data['category'] === 'Overdue' ? 'text-red-700' : 'text-green-700' }}">
                                    {{ $category }}
                                </div>
                            ...
                            <tbody>
                                @foreach($reportData['by_supplier'] as $item)
                                    <tr class="border-b">
                                        ...
                                        <td class="text-right font-semibold text-red-600">Rp {{ number_format($item['total_remaining'], 0, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <x-filament::card>
                <h3 class="text-lg font-semibold mb-4">Detail Hutang</h3>
                {{ $this->table }}
            </x-filament::card>
        @endif
    </div>
